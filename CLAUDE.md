...
- **メガ 4 件**（`pokemon-mega.json`）: `mega-absol-z`（あく/ゴースト、きれあじ、65/154/60/75/60/151、49kg）、`mega-garchomp-z`（ドラゴン、ふゆう、108/130/85/141/85/151、99kg）、`mega-lucario-z`（かくとう/はがね、はどうのぼうご、70/100/70/164/70/151、49.4kg）、`mega-baxcalibur`（ドラゴン/こおり、ねつこうかん、115/175/117/105/101/87、315kg）
  ...
  - `MegaToggle.megaFormLabel(mega)`: 複数形態ボタンのラベルは日本語メガ名から「メガ」と種族名を除いた残り（X/Y/Z）で生成し、残りが空なら「メガ」。旧実装は key 末尾を大文字化していたため Z 形態種で「メガLUCARIO」になっていた（`tests/presentation/megaFormLabel.test.ts`）
...
#### 背景
- オボン（HP50%以下で1/4回復）は「1発目でHPが半分を切った後・2発目の前」に回復する＝**2発の間に挟まる回復**
- これを背景の「定数回復」（末尾でまとめて適用）で入れると、撃破が終端扱いで先に確定するため回復が効かず、本来は乱数生存のところが確定撃破になる
  - 例: ブリジュラス流星群→C-2流星群 vs ガブリアス(オボン)。2発合計の最小がHPちょうどでも、間にオボン1/4が入れば生存圏に入る

#### 修正（UX）
- `DamageProgressionPanel.tsx`: 各攻撃イベント行に **「+回復」ボタン** を追加。クリックでその直後に防御側回復イベントを挿入（初期値=`floor(defenderMaxHp/4)`＝オボン相当）
- 背景「定数回復」欄に「末尾適用。条件回復は各攻撃行の +回復 で挟む」旨のヒントを追記
- これにより「Draco1 → +回復(オボン) → Draco2」を1クリックで構成でき、回復が正しいタイミング（攻撃の間）で適用される

#### 補足（仕様）
- 背景定数回復は「末尾でまとめて適用」のまま（残飯のような終端回復向け）。位置依存の回復は攻撃行の「+回復」または「+防御側回復」イベントで明示的に挟む
