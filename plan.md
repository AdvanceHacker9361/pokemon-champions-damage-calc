...
## 2026-09-10 V3.19.0 レギュレーション M-C 対応（Fable 司令塔・マルチエージェント）

### User Request

- 添付画像のロースター（メガ 6 体・通常 25 体＋イキリンコ 4 色）と持ち物 12 種を Reg.M-C として反映する。種族値・特性等の詳細はこちらで調査する。
- Fable を司令塔とし、Opus / Sonnet / Haiku をサブエージェントとして使い分けてトークンを最適化する。

### 実施体制

- Fable: 現状把握の統括、データ突合スクリプト、仕様確定、差分レビュー、ゴミエントリ削除・特性和訳の最終クリーンアップ、ドキュメント・リリース
- Sonnet ×5: plan/debug/CHANGELOG 要約、Web 調査（WebFetch はプロキシで遮断・WebSearch 要約のみ）、Showdown learnsets との習得技突合、データ追記（JSON 5 ファイル＋整合性テスト）
- Opus ×1: 計算エンジン・UI・テストの実装
- Haiku ×1: コード内の特性／持ち物参照の棚卸し

### 一次情報の扱い

- 種族値・タイプ・体重・技データ: Showdown `data/pokedex.ts` / `moves.ts`（raw.githubusercontent.com はプロキシ許可）。Legends Z-A 由来メガも収録済み
- 新メガの特性: 公式 X（@Poke_Champ_jp）の告知で確定（アブソルZ=きれあじ、ガブリアスZ=ふゆう、ルカリオZ=はどうのぼうご＝接触技の被ダメ半減）。はどうのぼうご の効果は Showdown の `auraguard`（contact ×0.5, breakable）と一致
- Web 要約に含まれた誤り（ねらいうち＝かくとう、ドラムアタック＝グソクムシャ等）は Showdown で棄却

### Current Status

- 実装・検証完了（typecheck / lint / vitest 31 ファイル 522 件 / build）。V3.19.0 としてブランチへコミット・プッシュ
- リリース前調整（バージョン据え置き）: スターアサルト名修正 / きょけんとつげき使用後状態（最終ダメ ×2・必中・攻守トグル・ダメージ進行の自動 2 倍、3 値スケール）/ がんじょうあご / コードレビュー指摘 5 件の修正（エクスポート反映・ゴツゴツメット接触判定・isMoldBreaker 集約）。最終 33 ファイル 554 件
- Web 突合（Sonnet）: 6 メガ・23 体・7 持ち物・14 技すべて公開情報と一致。M-C パッチの下方修正（ねがいごと・ちからをすいとる）は変化技のためダメージ計算に影響なし
- ユーザー指示により main へ fast-forward マージ・Pages デプロイを実施（2026-09-10）
- 既知の残課題: 和名化した特性のうち計算未実装のもの（がんじょうあご・こおりのりんぷん・トランジスタ・りゅうのあぎと・いわはこび・こだいかっせい／クォークチャージ・わざわい系 等）は別タスク
