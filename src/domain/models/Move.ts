...
export type SpecialMoveTag =
  ...
  | 'terrain-pulse' // だいちのはどう: フィールド中（接地時）はタイプがフィールド依存で威力2倍
...
export interface MoveData {
  ...
  critChance?: number
  /**
   * 吸収技の回復率（与えたダメージに対する割合）。
   * 例: ドレインパンチ/ギガドレイン/きゅうけつ = 0.5、ドレインキッス = 0.75。
   * バトルシーケンスで攻撃側（吸収）の回復量計算に使用。
   */
  drain?: number
  /**
   * 反動技の自傷率（実際に与えたダメージに対する割合）。
   * 例: ブレイブバード/フレアドライブ = 1/3、もろはのずつき = 1/2。
   */
  recoil?: number
}
