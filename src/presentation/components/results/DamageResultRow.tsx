...
function rollKoClass(roll: number, hp: number): string {
  if (roll >= hp) return 'text-red-400 font-bold'
  if (roll * 2 >= hp) return 'text-orange-400'
  if (roll * 3 >= hp) return 'text-yellow-400'
  ...
  return (
    <div className="mt-1 overflow-x-auto">
      <div className="text-xs text-slate-400 mb-1">
        おやこあい合計 (親 + 子×25%) — <span className="text-red-400">赤=確定KO</span>
        <span className="text-orange-400 ml-2">橙=乱数2発</span>
      </div>
      <table className="text-xs font-mono border-collapse">
        <thead>
          <tr>
            <th className="text-slate-600 pr-1 text-right">親↓子→</th>
            {childRolls.map((c, j) => (
        ...
        <tbody>
          {rolls.map((r, i) => (
            <tr key={i}>
              <td className="text-slate-400 pr-1 text-right">{r}</td>
              {childRolls.map((c, j) => {
                const total = r + c
                ...
                return (
                  <td
                    key={j}
                    className={`text-center px-0.5 ${isKo ? 'text-red-400 font-bold' : 'text-slate-500'}`}
                  >
                    {total}
                  </td>
                )
...
function ParentalBondKoInfo({ rolls, childRolls, defenderHp }: { rolls: number[]; childRolls: number[]; defenderHp: number }) {
  ...
  if (koCount === total) return <div className="text-xs text-red-400 mt-1">確定KO (親子愛1発)</div>
  return (
  ...
  return (
    <div className="py-2 border-b border-slate-800 last:border-0">
      ...
      <div className="flex items-center gap-2 mb-1.5 flex-wrap">
        <span className="text-sm font-mono text-slate-100">{min}〜{max}</span>
        ...
        <div className="mt-2 pt-2 border-t border-slate-700 space-y-2">
          ...
          <div className="bg-slate-800 rounded px-2 py-1.5 flex items-center justify-between">
            <div>
              ...
              <span className="text-sm font-mono text-slate-100">{multiMin}〜{multiMax}</span>
