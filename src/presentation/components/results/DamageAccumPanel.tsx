...
export function DamageAccumPanel() {
  ...
  // エントリがある場合は最初のエントリのdefenderMaxHpを基準にする
  ...
  return (
    <div className="panel space-y-2">
      <div className="flex items-center justify-between">
        <h3 className="text-xs font-semibold text-slate-300">加算計算リスト</h3>
        <button
          type="button"
      ...
      <div className="space-y-1">
        {entries.map(entry => (
          <div key={entry.id} className="flex items-center justify-between text-xs">
            <span className="text-slate-300 truncate flex-1">{entry.label}</span>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          {hp > 0 && (
            <span className="text-xs text-slate-600 ml-1">/{hp}</span>
          )}
        </div>
